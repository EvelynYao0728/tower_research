...
class FactorLibraryManager:
    ...
    def add_factors_from_experiment(
        ...
        for idx, task in enumerate(sub_tasks):
                ...
                if ws_path:
                    ...
                    for part in ws_path.parts:
                        if part.startswith("workspace_"):
                            ...
                            break
                    parquet_file = ws_path / "factor_output.parquet"
                    cache_location = {
                    ...
                    if parquet_file.exists():
                        cache_location["factor_output_parquet_path"] = str(parquet_file)
            ...
            if factor_expr and cache_location.get("factor_output_parquet_path"):
                self._sync_parquet_to_md5_cache(
                    factor_expr, cache_location["factor_output_parquet_path"]
                )
    ...
    @staticmethod
    def _sync_parquet_to_md5_cache(
        factor_expression: str,
        parquet_path: str,
        cache_dir: Optional[str] = None,
    ) -> bool:
        """Sync factor values from factor_output.parquet to MD5 cache dir (.pkl)."""
        ...
        try:
            ...
            result = pd.read_parquet(parq_file)
            result.to_pickle(pkl_file)
            logger.debug(f"Synced factor cache -> {pkl_file.name}")
            return True
        except Exception as e:
            logger.debug(f"Sync factor cache failed [{parquet_path}]: {e}")
            return False

    @staticmethod
    def check_cache_status(library_path: str,
                           cache_dir: Optional[str] = None) -> dict:
        """Check cache status for each factor in library."""
        cache_dir = Path(cache_dir or DEFAULT_FACTOR_CACHE_DIR)
        ...
        for fid, finfo in factors.items():
            ...
            parquet_path = cloc.get("factor_output_parquet_path", "")

            status = "need_compute"
            if parquet_path and Path(parquet_path).exists():
                status = "parquet_cached"
                parquet_cached += 1
            elif expr:
    ...
    def warm_cache_from_json(library_path: str,
                             cache_dir: Optional[str] = None) -> dict:
        """Walk factor library JSON and sync factor_output.parquet files to MD5 cache dir."""
        ...
        for fid, finfo in factors.items():
            ...
            if not expr or not parquet_path:
                no_source += 1
                skipped += 1
                continue
    ...
    def _extract_backtest_results(experiment) -> dict:
        """Extract backtest metrics from experiment (summary table or result Series)."""
        from quantaalpha.backtest.summary_metrics import (
            metrics_from_experiment_result,
            metrics_from_summary_dataframe,
            pick_best_factor_metrics,
        )

        factor_names = [
            getattr(t, "factor_name", "")
            for t in getattr(experiment, "sub_tasks", []) or []
            if getattr(t, "factor_name", None)
        ]
        run_summary = getattr(experiment, "backtest_summary", None)
        if isinstance(run_summary, pd.DataFrame) and not run_summary.empty and factor_names:
            per_factor = metrics_from_summary_dataframe(run_summary, factor_names)
            if per_factor:
                return pick_best_factor_metrics(per_factor)

        parsed = metrics_from_experiment_result(getattr(experiment, "result", None))
        if parsed:
            return parsed

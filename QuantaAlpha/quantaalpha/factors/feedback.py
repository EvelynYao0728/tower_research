...
def process_results(current_result, sota_result):
    ...
    def _important_metrics_for(df: pd.DataFrame) -> list[str]:
        idx = set(str(i) for i in df.index)
        if idx.intersection({"RankIC", "ICIR", "long_short_ret"}):
            return [m for m in _private_bt_metrics if m in idx]
        return [m for m in _qlib_style_metrics if m in idx]
    ...
    if sota_result is None or (isinstance(sota_result, pd.DataFrame) and sota_result.empty):
        ...
        important_metrics = _important_metrics_for(current_df)
        if important_metrics:
            filtered_df = current_df.loc[important_metrics]
        else:
    ...
    merged_order = _private_bt_metrics + _qlib_style_metrics
    picked = set(_important_metrics_for(current_df)) | set(_important_metrics_for(sota_df))
    important_metrics = [m for m in merged_order if m in picked]

...
def _coerce_bool(value: Any) -> bool:
    ...
    if s in ("true", "1", "yes", "accept", "approved", "pass", "correct", "success"):
        return True
    if s in ("false", "0", "no", "reject", "failed", "fail", "incorrect"):
        return False
    return "pass" in s or "accept" in s
...
def _parse_final_decision_response(text: str) -> tuple[bool, str]:
    """Parse PackyAPI/Claude evaluator JSON (tolerates aliases and minor format drift)."""
    data: dict = {}
    try:
    ...
    if isinstance(data, dict):
        for wrap in ("result", "evaluation", "response", "output"):
            inner = data.get(wrap)
    ...
    decision = _first_present(
        data,
        ("final_decision", "finalDecision", "decision", "approved", "accept", "pass"),
    )
    feedback = _first_present(
        data,
        ("final_feedback", "finalFeedback", "feedback", "message", "reason", "comment"),
    )

    if decision is None:
        m = re.search(
            r'(?:final[_\s-]?decision|"final_decision")\s*[:=]\s*(true|false|yes|no|1|0)',
            text,
            re.IGNORECASE,
        )
        if m:
    ...
    if feedback is None:
        for pat in (
            r'(?:final[_\s-]?feedback|"final_feedback")\s*[:=]\s*["\']([^"\']+)["\']',
            r'(?:final[_\s-]?feedback|"final_feedback")\s*[:=]\s*(.+?)(?:\n\n|\Z)',
        ):
            m = re.search(pat, text, re.IGNORECASE | re.DOTALL)
            if m:
                feedback = m.group(1).strip().strip('"').strip("'")
                break
        if feedback is None and decision is not None:
            feedback = "See evaluator response."

    if decision is None:
        raise KeyError("final_decision")
    if feedback is None:
        raise KeyError("final_feedback")

    return _coerce_bool(decision), str(feedback).strip()
...
class FactorEvaluator:
    ...
    def _get_df(self, gt_implementation: Workspace, implementation: Workspace):
        if gt_implementation is not None:
            cached_gt = getattr(gt_implementation, "_last_execute_df", None)
        ...
        cached_gen = getattr(implementation, "_last_execute_df", None)
        ...
        while attempts < max_attempts:
            try:
                ...
                if attempts > 0:
                    up = (
                        up
                        + '\n\nYour previous reply was invalid. Output ONLY JSON: '
                        '{"final_decision": true, "final_feedback": "single line of text"}'
                    )
                resp = api.build_messages_and_create_chat_completion(
                    user_prompt=up,
                    system_prompt=system_prompt,
                ...
                return _parse_final_decision_response(resp)

...
def _try_evaluate_via_research_backtest(target_task: FactorTask) -> tuple[str | None, str | None, bool]:
    """
    若磁盘上已有该因子的 parquet 输入，则在 evaluate 中直接调用 ``research/backtest``，
    跳过 ``implementation.execute()``（不生成、不依赖 ``factor_output.parquet``）。

    Returns
    -------
    (execution_feedback, value_feedback, used_backtest)
        ``used_backtest`` 为 True 时调用方 **不得** 再执行 ``execute()``。
    """
    import os

    if os.environ.get("QUANTALPHA_EVALUATE_WITH_RESEARCH_BACKTEST", "0").lower() in ("0", "false", "no"):
    ...
    try:
        ...
        from quantaalpha.factors.private_runner import _resolve_disk_backtest_input
    except Exception as e:
        logger.warning("evaluate-stage research backtest bridge import failed: %s", e)
        return None, None, False

    cfg = PrivateDataConfig()
    name = target_task.factor_name
    ...
    try:
        if resolved[0] == "per_feature":
            bt = run_backtest(resolved[1], use_cache=True)
        else:
            bt = run_backtest(resolved[1], factor_col=resolved[2], use_cache=True)
    except Exception as e:
        logger.warning("evaluate-stage run_backtest failed, fallback to execute(): %s", e)
        return None, None, False

    paths = backtest_output_artifact_paths(bt.output_dir, name)
    path_lines = "\n".join(f"  {k}: {v}" for k, v in paths.items())
    if bt.summary.empty:
        summ = "(summary 为空)"
    else:
        sub = bt.summary[bt.summary["factor"].astype(str) == str(name)]
        summ = (
            sub.iloc[0].to_string()
            if not sub.empty
            else bt.summary.to_string(index=False)
        )[:5000]

    exec_fb = (
        f"[research/backtest] evaluate 阶段已直接完成单因子回测（未跑 factor.py / 无 factor_output.parquet）。\n"
        f"因子: {name}\n标准产物路径:\n{path_lines}\n\nsummary:\n{summ}"
    )
    val_fb = (
        "Value 评价已由 research/backtest 的 summary 替代（无 execute 返回的因子 DataFrame）。\n" + summ
    )
    return exec_fb, val_fb, True
    ...
    def evaluate(
        ...
        **kwargs,
        ...
        if (
            ...
            # 1. 优先：磁盘 parquet + research/backtest（可不跑 factor.py）
            exec_bt, val_bt, used_bt = _try_evaluate_via_research_backtest(target_task)
            if used_bt and exec_bt is not None:
                execution_feedback = exec_bt
                gen_df = None
            else:
                (
                    execution_feedback,
                ...
                if gen_df is not None:
                    implementation._last_execute_df = gen_df
            ...
            if used_bt and exec_bt is not None:
                factor_feedback.value_feedback = val_bt or ""

...
class FactorRegulator(Evaluator):
    ...
    def fields_are_loadable(self, expression: str) -> tuple[bool, str]:
        """True when every ``$field`` exists in the feature catalog and is not a backtest label."""
        return validate_factor_expression_fields(expression)
    ...
    def is_expression_acceptable(self, eval_dict) -> bool:
        ...
        # Condition 2/3: cap literal-argument and distinct-$field density vs tree size.
        # Use ratio <= 0.5 (with epsilon): the old strict check ``-log(1-x) < 0.693`` rejected x==0.5
        # because -log(0.5) == log(2) > 0.693 float literal, e.g. RANK($spread_mean) never passed.
        _half = 0.5 + 1e-12
        cond2 = free_args_ratio <= _half
        cond3 = unique_vars_ratio <= _half
    ...
    def add_factor(self, factor_name: str, factor_expression: str) -> bool:
        ...
        new_factor = pd.DataFrame(
            [{"factor_name": factor_name, "factor_expression": factor_expression}]
        )

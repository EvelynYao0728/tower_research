...
def _coerce_factor_row(item: dict) -> dict:
    row = dict(item)
    if not str(row.get("expression") or "").strip():
        for alt in ("expr", "factor_expression", "formula"):
            v = row.get(alt)
            if v and str(v).strip() and str(v).strip() != ".":
                row["expression"] = str(v).strip()
                break
    return row


def _normalize_factor_experiment_dict(data: dict | list | Any) -> dict:
    """LLM may return a list of factors or a name-keyed dict; normalize to dict."""
    if isinstance(data, dict):
        if "factors" in data and isinstance(data["factors"], (dict, list)):
            return _normalize_factor_experiment_dict(data["factors"])
        out: dict = {}
        for k, v in data.items():
            if isinstance(v, dict):
                out[str(k)] = _coerce_factor_row(v)
            else:
                out[str(k)] = v
        return out
    if isinstance(data, list):
        ...
        for i, item in enumerate(data):
            ...
            name = (
                item.get("name")
                or item.get("factor_name")
                or item.get("factor")
                or f"factor_{i + 1}"
            )
            out[str(name)] = _coerce_factor_row(item)
        return out
    return {}
...
class QlibFactorHypothesis2Experiment(FactorHypothesis2Experiment):
    ...
    def convert_response(self, response: str, trace: Trace) -> FactorExperiment:
        response_dict = _normalize_factor_experiment_dict(robust_json_parse(response))
        tasks = []
        ...
        for factor_name in response_dict:
            ...
            if not isinstance(factor_data, dict):
                continue
            row = _coerce_factor_row(factor_data)
...
class AlphaAgentHypothesisGen(FactorHypothesisGen):
    ...
    def gen(self, trace: Trace) -> AlphaAgentHypothesis:
                ...
                for attempt in range(max_json_retries):
                    try:
                        ...
                        resp = APIBackend().build_messages_and_create_chat_completion(
                            up, system_prompt, json_mode=json_flag, reasoning_flag=False
                        )
                        return self.convert_response(resp)
                    except json.JSONDecodeError as je:
                        last_err = je
                        logger.warning(
                            "hypothesis JSON parse failed (attempt %d/%d): %s",
                            attempt + 1,
                            max_json_retries,
                            je,
                        )
        ...
        resp = APIBackend().build_messages_and_create_chat_completion(
            user_prompt, system_prompt, json_mode=json_flag, reasoning_flag=False
        )
...
class AlphaAgentHypothesis2FactorExpression(FactorHypothesis2Experiment):
    def __init__(
        ...
        *args,
        ...
        factors_per_hypothesis: int = 5,
        **kwargs,
    ):
        super().__init__(*args, **kwargs)
        self.factors_per_hypothesis = max(1, int(factors_per_hypothesis))
    ...
    def _convert_with_history_limit(self, hypothesis: Hypothesis, trace: Trace, history_limit: int) -> Experiment:
        ...
        system_prompt = (
            ...
            .render(
                ...
                factors_per_hypothesis=self.factors_per_hypothesis,
            )
        )
        user_prompt = (
            Environment(undefined=StrictUndefined)
        ...
        _max_regen_rounds = 40
        _regen_round = 0
        while True:
            if flag:
                break
            _regen_round += 1
            if _regen_round > _max_regen_rounds:
                raise RuntimeError(
                    "hypothesis2experiment: exceeded regeneration limit; "
                    "last factors may fail parsability, complexity gate, or LLM JSON. "
                    "Check logs and factor expressions."
                )
            ...
            try:
                response_dict = _normalize_factor_experiment_dict(robust_json_parse(resp))
            except json.JSONDecodeError as e:
                logger.warning(f"JSON parse failed: {e}, retrying...")
                continue
            if not response_dict:
                logger.warning("hypothesis2experiment: empty factor dict, retrying...")
                continue
            proposed_names = []
            proposed_exprs = []
            ...
            for factor_name in response_dict:
                ...
                if not isinstance(factor_data, dict):
                    continue
                expr = (
                    factor_data.get("expression")
                    or factor_data.get("expr")
                    or factor_data.get("factor_expression")
                    or ""
                )
                if not str(expr).strip() or str(expr).strip() == ".":
                    logger.warning(
                        "hypothesis2experiment: empty expression for %s keys=%s",
                        factor_name,
                        list(factor_data.keys()),
                    )
                    break
                ...
                loadable, load_msg = self.factor_regulator.fields_are_loadable(expr)
                if not loadable:
                    logger.info(f"Expression fields not loadable: {load_msg}")
                    if expression_duplication_prompt is None:
                        expression_duplication_prompt = load_msg
                    else:
                ...
                if self.consistency_enabled and self.quality_gate is not None:
                    try:
                        ...
                        if results.get("corrected_expression") and results["corrected_expression"] != expr:
                            ...
                            loadable, load_msg = self.factor_regulator.fields_are_loadable(expr)
                            if not loadable:
                                logger.warning(f"Corrected expression uses invalid fields: {load_msg}")
                                break
                            success, eval_dict = self.factor_regulator.evaluate(expr)
                            if not success:
                                break
                ...
                n_valid = len(proposed_names)
                target = self.factors_per_hypothesis
                if n_valid >= target:
                    if n_valid > target:
                        logger.info(
                            "hypothesis2experiment: got %d factors, using first %d",
                            n_valid,
                            target,
                        )
                        proposed_names = proposed_names[:target]
                        proposed_exprs = proposed_exprs[:target]
                        response_dict = {
                            k: response_dict[k] for k in proposed_names if k in response_dict
                        }
                        resp = json.dumps(response_dict, ensure_ascii=False)
                    flag = True
                else:
                    logger.warning(
                        "hypothesis2experiment: need at least %d factors, got %d; retrying...",
                        target,
                        n_valid,
                    )

        # Add valid factors to the factor regulator (one row per factor)
        for _nm, _ex in zip(proposed_names, proposed_exprs, strict=True):
            self.factor_regulator.add_factor(_nm, _ex)
    ...
    def convert_response(self, response: str, trace: Trace) -> FactorExperiment:
        ...
        exp.tasks = unique_tasks
        if tasks and not unique_tasks:
            logger.warning(
                "convert_response: %d factor(s) dropped as duplicates; names=%s",
                len(tasks),
                [t.factor_name for t in tasks],
            )
        elif not unique_tasks:
            logger.warning(
                "convert_response: no factors parsed from LLM response (len=%d)",
                len(response or ""),
            )
        return exp

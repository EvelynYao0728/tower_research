...
class MultiProcessEvolvingStrategy(EvolvingStrategy):
    ...
    def evolve(
        ...
        **kwargs,
        ...
        qk = queried_knowledge if queried_knowledge is not None else CoSTEERQueriedKnowledge()
        # Find tasks to evolve
        to_be_finished_task_index = []
        for index, target_task in enumerate(evo.sub_tasks):
            target_task_desc = target_task.get_task_information()
            if target_task_desc in qk.success_task_to_knowledge_dict:
                evo.sub_workspace_list[index] = qk.success_task_to_knowledge_dict[
                    target_task_desc
                ].implementation
            elif (
                target_task_desc not in qk.success_task_to_knowledge_dict
                and target_task_desc not in qk.failed_task_info_set
            ):
        ...
        if self.settings.select_threshold < len(to_be_finished_task_index):
            ...
            to_be_finished_task_index = self.select_one_round_tasks(
                to_be_finished_task_index, evo, self.settings.select_threshold, qk, self.scen
            )

...
def apply_run_config_env(run_cfg: dict[str, Any]) -> None:
    """将 experiment.yaml 中的开关同步到 os.environ（供 CoSTEER / evaluators 读取）。"""
    if not isinstance(run_cfg, dict):
        return
    bt = run_cfg.get("backtest") or {}
    if not isinstance(bt, dict):
        return
    if bt.get("evaluate_with_research_backtest") is not None:
        val = "1" if bool(bt["evaluate_with_research_backtest"]) else "0"
        os.environ["QUANTALPHA_EVALUATE_WITH_RESEARCH_BACKTEST"] = val

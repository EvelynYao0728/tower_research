...
# Set in AlphaAgentLoop.__init__ / load / run; None when not used.
STOP_EVENT: threading.Event | None = None


def _apply_costeer_knowledge_flags(loop: "AlphaAgentLoop") -> None:
    """续跑时 pickle 里的 CoSTEER 可能仍 with_knowledge=True，按当前配置同步。"""
    from quantaalpha.factors.coder.config import FACTOR_COSTEER_SETTINGS

    coder = getattr(loop, "coder", None)
    if coder is None:
        return
    for attr in ("with_knowledge", "knowledge_self_gen"):
        val = getattr(FACTOR_COSTEER_SETTINGS, attr, None)
        if val is not None:
            setattr(coder, attr, val)
    agent = getattr(coder, "evolve_agent", None)
    if agent is not None:
        agent.with_knowledge = FACTOR_COSTEER_SETTINGS.with_knowledge
        agent.knowledge_self_gen = FACTOR_COSTEER_SETTINGS.knowledge_self_gen
...
def stop_event_check(func):
    ...
    def wrapper(self, *args, **kwargs):
        ev = getattr(self, "stop_event", None) or STOP_EVENT
...
class AlphaAgentLoop(LoopBase, metaclass=LoopMeta):
    ...
    def __init__(
        ...
        with logger.tag("init"):
            ...
            self.evolution_phase = evolution_phase  # original
    ...
    @classmethod
    def load(cls, path, use_local: bool = True, stop_event: threading.Event | None = None):
        ...
        global STOP_EVENT
        STOP_EVENT = stop_event
        _apply_costeer_knowledge_flags(instance)
    ...
    def factor_calculate(self, prev_out: dict[str, Any]):
        ...
        with logger.tag("d"):  # develop
            ...
            if FACTOR_COSTEER_SETTINGS.promote_full_panel_after_debug:
                promote_experiment_after_debug(factor)
